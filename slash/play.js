...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (interaction.options.getSubCommands() === "song") {
            ...
            embed
                .setDescription(
                    `Song ${song.title} has been added to the queue!`
                )
                .setThumbnail(song.thumbnail)
                .setFooter({ text: "Duration " + song.duration });
        } else if (interaction.options.getSubCommands() === "playlist") {
            let url = interaction.options.getString("url");
            const result = await client.player.search(url, {
                requestedBy: interaction.user,
        ...
        } else if (interaction.options.getSubCommands() === "search") {
            let url = interaction.options.getString("url");
            const result = await client.player.search(url, {
                requestedBy: interaction.user,
